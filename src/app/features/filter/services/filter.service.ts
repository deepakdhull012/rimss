...
export class FilterService implements OnDestroy {
  ...
  getPriceBreakPoint(): Observable<Array<number>> {
    return of([500, 1000, 1500, 2000, 5000]);
  }

  getDiscountBreakPoints(): Observable<Array<number>> {
    return of([10,20,30,40,50]);
  }

  getSizes(): Observable<Array<string>> {
  ...
  getFilterString(filterObj: IFilterObject): string {
    let filterString = "";
  ...
  private appendBrandString(initialFilterString: string, brandIds: Array<number>): string {
    for (const brandId of brandIds) {
      initialFilterString+= `brandId=${brandId}&`
    }
    initialFilterString = initialFilterString.substring(0, initialFilterString.length-1);
    return initialFilterString;
  }

  private appendPriceString(initialFilterString: string, priceRanges: Array<IPriceRange>): string {
  ...
  private appendRatingString(initialFilterString: string, ratings: Array<number>): string {
  ...
  private appendSizeString(initialFilterString: string, sizes: Array<string>): string {
  ...
  private appendDiscountString(initialFilterString: string, discounts: Array<number>): string {
